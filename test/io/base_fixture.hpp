...
struct hdu_store {
    ...
    hdu_store() {}
    hdu_store(const header& head, const std::string& data_buffer)
        :hdu_header(head), hdu_data_buffer(data_buffer) {}   
};

template<typename FileReader>
class base_fixture {
    typedef std::map<std::string, hdu_store> hdus_information;
...
public:
    ...
    hdu_store* get_raw_hdu(const std::string& sample_name, const std::string& hdu_name) {
...
private:
    ...
    void read_hdus(const std::string& sample_name) {
        while (!file_reader.at_end()) {
            ...
            samples[sample_name][hdu_header.get_hdu_name()] = hdu_store(hdu_header, hdu_data);
        }
    }
